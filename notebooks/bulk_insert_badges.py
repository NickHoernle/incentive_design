...
SQL = f"INSERT INTO badges ({','.join(fields)}) VALUES ({','.join(['%s' for f in fields])});"
done_to = 250005
with open('/home/nick/Downloads/stack_overflow/Badges.xml', 'r') as f:
    line = f.readline()
    ...
    while line:
        ...
        FIELDS = [xml[f'@{field}'] if f'@{field}' in xml else '' for field in fields]
        FIELDS[-1] = 1 if FIELDS[-1] == 'True' else 0
        FIELDS = tuple(FIELDS)
        to_insert.append(FIELDS)

        if len(to_insert) == 250000:
